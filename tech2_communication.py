...
def download_tech2_data(port_name, output_file=None, download_only_seed=False):
    ...
    try:
        ...
        if data_buffers:
            ...
            # Extract seed data from the combined data
            if download_only_seed and len(combined_data) >= 0x32:
...
def get_seed_only(port_name):
    """Get only the seed value from the Tech2"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download data from Tech2 diagnostic device")
    parser.add_argument("-p", "--port", required=True, help="Serial port (e.g., COM5)")
    parser.add_argument("-o", "--output", default="tech2_data.bin", help="Output file name")
    parser.add_argument("-s", "--seed-only", action="store_true", help="Get only the seed value")
    parser.add_argument("-r", "--restart", action="store_true", help="Send restart command only")
    args = parser.parse_args()
    
    if args.restart:
        # Just send a restart command and exit
        with open_tech2_port(args.port) as port:
            send_restart_command(port)
            
    if args.seed_only:
        seed = get_seed_only(args.port)
        if seed is not None:
            print(f"Seed value: 0x{seed:04X}")
        else:
            print("Failed to get seed value")
    else:
